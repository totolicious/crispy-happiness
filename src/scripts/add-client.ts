...
`);
  ...
  const inputpreferentialComissionEur =
    process.argv[3] === "a" ? undefined : process.argv[3];
  ...
  try {
    ...
    if (id && preferentialComissionEur) {
      result = await connection.query(
        'INSERT INTO "client"("id", "preferentialComissionEur") VALUES ($1, $2) RETURNING "id"',
        [id, preferentialComissionEur]
      );
    } else if (id) {
      result = await connection.query(
        'INSERT INTO "client"("id") VALUES ($1) RETURNING "id"',
        [id]
      );
    } else if (preferentialComissionEur) {
      result = await connection.query(
        'INSERT INTO "client"("preferentialComissionEur") VALUES ($1) RETURNING "id"',
        [preferentialComissionEur]
      );
    } else {
      result = await connection.query(
        'INSERT INTO "client"("preferentialComissionEur") VALUES(NULL) RETURNING "id"'
      );
    }

    logger.info(
      `Client '${result[0].id}' was added with preferred commission '${preferentialComissionEur}'`
    );
